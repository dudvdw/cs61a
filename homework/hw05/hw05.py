...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        delta_year =  Mint.current_year - self.year - 50
        if delta_year > 0:
            return self.cents + delta_year
        else:
            return self.cents
...
def store_digits(n):
    ...
    if n < 10:
        ...
        # log_n = math.floor(math.log10(n))
        # digit_n = math.pow(10, log_n)
        # first = math.floor(n // digit_n)
        # rest = math.floor(n - first * digit_n)
        # return Link(first, store_digits(rest))
        digit = 1
        while digit <= n:
            digit *= 10
        digit = digit//10
        return Link(n//digit, store_digits(n%digit))
