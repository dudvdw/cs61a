...
def swine_align(player_score, opponent_score):
    ...
    if player_score==0 or opponent_score==0:
        ...
        while abs(player_score-opponent_score) !=0:
            delta = abs(player_score - opponent_score)
            if player_score > opponent_score:
                player_score = delta
            else:
                opponent_score = delta
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal and score1 < goal):
        if who==0:
            ...
            score0 += take_turn(roll_num0, score1, dice)
            say = say(score0, score1)
            if not extra_turn(score0, score1):
                who = 1 
        else:
            roll_num1 = strategy1(score1, score0)
            score1 += take_turn(roll_num1, score0, dice)
            say = say(score0, score1)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who==0:
            increase_score, new_last_score = score0 - last_score, score0
        elif who==1:
            increase_score, new_last_score = score1 - last_score, score1
        if increase_score>running_high:
            print(increase_score, 'point(s)! The most yet for Player', who)
            new_running_high = increase_score
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    n, highest_score, dice_num = 1, 0, 1
    average_score = make_averaged(roll_dice)
    while n <= 10:
        score = average_score(n, dice)
        if score>highest_score:
            highest_score, dice_num = score, n
        n += 1
    return dice_num
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    free_score = free_bacon(opponent_score)
    is_extra = extra_turn(score+free_score, opponent_score)
    if is_extra:
        return 0
    else:
